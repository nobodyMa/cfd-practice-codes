classdef collocated_segregated
    ...
    methods (Static)
        ...
        function coef_tensor = conduction_coef(case_obj, dim, ncx, ncy, ncz, ncoef, dt, spht, con, ...
            ...
            aE(:,:,:) = solver.a_nb(area_x, idx, ul, ur, mul, mur, rho_e(:,:,:), -1.0);
            % 西方向系数
            ul = uf(1:end-1,:,:); ur = uf(1:end-1,:,:);
            aW(:,:,:) = solver.a_nb(area_x, idx, ul, ur, mul, mur, rho_w(:,:,:), 1.0);

            % 北方向系数
            vl = vf(:,2:end,:); vr = vf(:,2:end,:);
            aN(:,:,:) = solver.a_nb(area_y, idy, vl, vr, mul, mur, rho_n(:,:,:), -1.0);
            % 南方向系数
            vl = vf(:,1:end-1,:); vr = vf(:,1:end-1,:);
            aS(:,:,:) = solver.a_nb(area_y, idy, vl, vr, mul, mur, rho_s(:,:,:), 1.0);

            % 3D情况
            if dim == 3
                % 顶面系数
                wl = wf(:,:,2:end); wr = wf(:,:,2:end);
                aT(:,:,:) = solver.a_nb(area_z, idz, wl, wr, mul, mur, rho_t(:,:,:), -1.0);
                % 底面系数
                wl = wf(:,:,1:end-1); wr = wf(:,:,1:end-1);
                aB(:,:,:) = solver.a_nb(area_z, idz, wl, wr, mul, mur, rho_b(:,:,:), 1.0);
            end
